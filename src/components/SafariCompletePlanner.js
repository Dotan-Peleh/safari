...
const SafariCompletePlanner = () => {
  ...
  const routes = {
    ...
    proposal_3: {
      ...
      pricePerPerson: 5818,
      color: "#FF9800",
      description: "שילוב של תנזניה 7 ימים + אוגנדה 7 ימים - 14 ימים מלאים",
      itinerary: [
        ...
        { day: 7, location: "entebbe", nights: 1, highlight: false, flight: true },
        { day: 8, location: "kibale", nights: 1, highlight: "🐒" },
        { day: 9, location: "queen_elizabeth", nights: 2, highlight: "🦁" },
        { day: 11, location: "bwindi", nights: 2, highlight: "🦍" },
        { day: 13, location: "entebbe", nights: 1, highlight: false },
        { day: 14, location: "entebbe", nights: 0, highlight: false }
      ],
      pros: [
        "שילוב של שתי המדינות - תנזניה + אוגנדה",
        "גורילות + שימפנזים + Big 5 מלא",
        "חציית נהר המארה - הנדידה הגדולה",
        "קרנף שחור בנגורונגורו",
        "טרנגירה - 5,000+ פילים",
        "שייט קזינגה + אריות מטפסי עצים",
        "מחיר סביר ($5,818 לאדם)"
      ],
      cons: [
        "14 ימים - דורש זמן",
        "ללא טיסה פנימית - נסיעות ארוכות",
        "מחיר גבוה יותר מהצעה 4 (ללא טיסה)"
      ],
      uniqueFeatures: [
        "🌊 חציית המארה + גורילות + שימפנזים",
        "🚤 שייט קזינגה + אריות מטפסי עצים",
        "🦏 קרנף שחור + טרנגירה - Big 5 מלא",
        "💰 מחיר סביר ללא טיסה פנימית"
      ]
    },
    proposal_3_updated: {
      name: "הצעה 3 מעודכנת",
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', background: 'linear-gradient(180deg, #1a1a2e 0%, #16213e 100%)', minHeight: '100vh', direction: 'rtl', color: 'white' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '15px' : '25px', padding: isMobile ? '15px' : '25px', maxWidth: '1500px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '20px', padding: isMobile ? '15px' : '25px', backdropFilter: 'blur(10px)' }}>
          ...
          {/* Compare Tab */}
          {activeTab === 'compare' && (
            <div style={{ maxHeight: isMobile ? 'none' : '600px', overflowY: isMobile ? 'visible' : 'auto', padding: isMobile ? '5px' : '10px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  {Object.entries(routes).map(([key, route]) => {
                    const comparisonData = [
                      { label: '📅 ימים', value: route.days },
                      { label: '💰 מחיר לאדם', value: `$${route.pricePerPerson.toLocaleString()}` },
                      { label: '✈️ עם טיסה', value: key === 'proposal_4_v1' ? '$13,600' : '-' },
                      { label: '🗺️ מדינות', value: '2 (תנזניה + אוגנדה)' },
                      { label: '🦍 גורילות', value: '✅' },
                      { label: '🐒 שימפנזים', value: '✅' },
                      { label: '🌊 חציית המארה', value: (key === 'proposal_4_v1' || key === 'proposal_3' || key === 'proposal_3_updated') ? '✅' : '❌' },
                      { label: '🦏 קרנף שחור', value: '✅' },
                      { label: '🏖️ זנזיבר', value: key === 'proposal_4_v2' ? '✅' : '❌' },
                      { label: '🦁 אריות מטפסים', value: (key === 'proposal_4_v2' || key === 'proposal_3' || key === 'proposal_3_updated') ? '✅' : '❌' },
                      { label: '🐘 טרנגירה', value: (key === 'proposal_4_v1' || key === 'proposal_3' || key === 'proposal_3_updated') ? '✅' : '❌' },
                      { label: '🚤 שייט קזינגה', value: (key === 'proposal_4_v2' || key === 'proposal_3' || key === 'proposal_3_updated') ? '✅' : '❌' },
                      { label: '🦁 Big 5 מלא', value: '✅' },
                      { label: '✈️ טיסה פנימית', value: key === 'proposal_4_v1' ? '✅' : '❌' },
                      { label: '📅 יום נוסף סרנגטי', value: key === 'proposal_3_updated' ? '✅' : '❌' }
                    ];
                    
                    return (
                      <div key={key} style={{ 
                        background: `linear-gradient(135deg, ${route.color}20, ${route.color}10)`, 
                        borderRadius: '15px', 
                        padding: '15px', 
                        border: `2px solid ${route.color}40` 
                      }}>
                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px', gap: '10px' }}>
                          <div style={{ fontSize: '1.5em' }}>{key === 'proposal_4_v1' && '⭐'}</div>
                          <div>
                            <h4 style={{ margin: 0, color: route.color, fontSize: '1.1em', fontWeight: 'bold' }}>{route.name}</h4>
                            <p style={{ margin: '3px 0 0', fontSize: '0.85em', opacity: 0.8 }}>{route.subtitle}</p>
                          </div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '0.9em' }}>
                          {comparisonData.map((item, idx) => (
                            <div key={idx} style={{ 
                              ...
                              <span style={{ fontWeight: 'bold', color: item.value === '✅' ? '#48bb78' : item.value === '❌' ? '#f56565' : route.color }}>
                                {item.value}
                              </span>
                            </div>
                          ))}
